...
// just for ugly constants/macros in nat_const, remove when saturate code is cleaner
...
//    cout << " running _select_deephom on vr=" << vr << " vl= " ; vl.set_print(cout) ; cout << endl ;
    ...
    if (vr == SUCCARG || vr == LEFT || vr == RIGHT) {
      vl2 = saturateNat(vl2);
    }
    if (vr == type_condition_) {
      SDD subresult = condition_(vl2);
...
GShom extract_value (int trigger, const GShom & extractor) {
