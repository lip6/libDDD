...
public:
	...
	GSDD eval(const GSDD &d) const {
		...
		for (GSDD::const_iterator it1 = d.begin(); it1 != d.end(); ++it1) {
			for (GSDD::const_iterator it2 = value.begin(); it2 != value.end();
					++it2) {

				// downcast this garbage to DDD
				DDD newval = apply2k(*(const DDD*) it2->first)(
						*(const DDD*) it1->first);
				if (newval != DDD::null) {
					toadd.insert(
							GSDD(d.variable(), newval,
									apply2k(it2->second)(it1->second)));
				}
			}
		}
...
public:
	...
	GSDD eval(const GSDD &d) const {
		...
		if (left != GShom::id && left.skip_variable(variable)) {
			...
			for (GSDD::Valuation::const_iterator it = d.begin(); it != d.end();
					++it) {
				// for each son of p2 the constant :   v - b -> s2
				for (GSDD::Valuation::const_iterator jt = right.begin();
						jt != right.end(); ++jt) {
					...
					if (it->first->set_equal(*jt->first)) {
						...
						square_union(res, (left * jt->second)(it->second),
								it->first);
...
class SDomExtract: public _GShom {
...
class LocalApply: public _GShom {
...
class SLocalApply: public _GShom {
...
//      assert(p.size() > 1);
	...
	size_t hash() const {
		size_t res = 40693;
		for (parameters_it gi = parameters.begin(); gi != parameters.end();
				++gi)
			res ^= gi->hash();
		return res;
	}
	...
	void factorizeByLevel(Gset_t &G, int target) const {
		...
		for (Gset_it it = G.begin(); it != G.end(); ++it) {
			...
			if (const And *hand = dynamic_cast<const And*>(get_concret(*it))) {
				...
				bool niceform = false;
				for (And::parameters_it gi = hand->parameters.begin();
						gi != hand->parameters.end(); ++gi) {
					if (!gi->skip_variable(target)) {
						...
						} else if (dynamic_cast<const SLocalApply*>(get_concret(
								*gi))) {
				...
				if (niceform) {
					...
					std::pair<map_it, bool> insertion = map_ltof.insert(
							map_t::value_type(l, sf));
					if (insertion.second) {
...
//	      trace << "factorization rule 1 successful ! \n" ;
		...
		for (map_it it = map_ltof.begin(); it != map_ltof.end(); ++it) {
			...
			tmp.insert(it->first);
			std::pair<map_it, bool> insertion = map_ftol.insert(
					map_t::value_type(GShom::add(it->second), tmp));
			if (insertion.second) {
				// did not exist, continue;
			} else {
				// already in map : Apply factorization rule
...
public:
	...
	Add(const d3::set<GShom>::type &p, bool have_id) :
			parameters(p.begin(), p.end()), have_id(have_id) {
	}
...
public:
	...
	bool skip_variable(int var) const {
		return sat.skip_variable(var) && lf.skip_variable(var);
	}
			...
			if (const Add *add = dynamic_cast<const Add*>(get_concret(sat))) {
				...
				if (add->get_have_id()) {
					...
					if (partition.has_local) {
						if (const LocalApply *loc =
								dynamic_cast<const LocalApply*>(get_concret(
										*partition.L))) {
							// Hom/DDD case
							GHom hh = fixpoint(GHom(loc->h));
							Lsatd3 = hh & ltermd3;
						} else if (const SLocalApply *sloc =
								dynamic_cast<const SLocalApply*>(get_concret(
										*partition.L))) {
...
public:
	...
	bool skip_variable(int var) const {
		return left.skip_variable(var) && right.skip_variable(var);
	}
...
public:
	...
	/* Compare */
	bool operator==(const _GShom &h) const {
		return left == ((LeftConcat*) &h)->left
				&& right == ((LeftConcat*) &h)->right;
	}
	size_t hash() const {
		return 23 * left.hash() + 47 * right.hash();
	}
	_GShom* clone() const {
		return new LeftConcat(*this);
	}
...
public:
	...
	/* Compare */
	bool operator==(const _GShom &h) const {
		return left == ((RightConcat*) &h)->left
				&& right == ((RightConcat*) &h)->right;
	}
	size_t hash() const {
		return 47 * left.hash() + 19 * right.hash();
	}
	_GShom* clone() const {
		return new RightConcat(*this);
	}
...
public:
	...
	/* Compare */
	bool operator==(const _GShom &h) const {
		return left == ((HomMinus*) &h)->left
				&& right == ((HomMinus*) &h)->right;
	}
	size_t hash() const {
		return 15971 * left.hash() + 24023 * right.hash();
	}
	_GShom* clone() const {
		return new HomMinus(*this);
	}
	...
	GShom invert(const GSDD &pot) const {
		...
		std::cerr
				<< "/!\\ Be careful, you are using invert of a minus between two homomorphisms, and I am not sure the invert is correct. FIXME !"
				<< std::endl;
...
public:
	...
	Fixpoint(const GShom &a, int ref = 0, bool can_garbage = false) :
			_GShom(ref), arg(a), can_garbage(can_garbage) {
	}
	/* Compare */
	...
	GSDD has_image(const GSDD &d) const {
		if (d == GSDD::null) {
			...
			if (const sns::Inter *inter =
					dynamic_cast<const sns::Inter*>(_GShom::get_concret(arg))) {
				if (inter->right == GShom::id) {
					if (const sns::Add *add =
							dynamic_cast<const sns::Add*>(_GShom::get_concret(
									inter->left))) {

						const sns::Add::partition &partition =
								add->get_partition(d.variable());
						...
						if (img != GSDD::null) {
							std::cerr
									<< "Fast SCC detection found an SCC at level "
									<< d.variable() << std::endl;
							return img;
						}
						...
						if (partition.has_local) {
							if (const LocalApply *loc =
									dynamic_cast<const LocalApply*>(partition.L)) {
								// Hom/DDD case
								GHom hh = fixpoint(GHom(loc->h) * GHom::id,
										true);
								...
								GShom hh =
										fixpoint(
												GShom(
														((const SLocalApply*) partition.L)->h)
														* GShom::id, true);
								L_part = localApply(hh, d.variable());
							}
						}
						...
						if (img != GSDD::null) {
							std::cerr
									<< "Fast SCC detection found a local SCC at level "
									<< d.variable() << std::endl;
							return img;
						}
					}
		...
		if (d == GSDD::null) {
			...
			if (const Add *add = dynamic_cast<const Add*>(get_concret(arg))) {
				...
				if (add->get_have_id()) {
					...
					if (can_garbage && partition.F == GShom::id
							&& partition.G.empty()) {
					...
					if (GShom::getSaturationStrategy()
							== GShom::RECFIREANDSAT) {
						...
						for (Add::Gset_it G_it = partition.G.begin();
							...
							trace << "testing g element : " << *G_it
									<< std::endl;
							// find elements of the form l & f
							// test if *it of the form l & f
							if (const And *hand =
									dynamic_cast<const And*>(get_concret(*G_it))) {
								bool niceform = true;

								for (And::parameters_it gi =
										hand->parameters.begin();
										gi != hand->parameters.end(); ++gi) {
									if (dynamic_cast<const LocalApply*>(get_concret(
											*gi))) {
										continue;
									} else if (dynamic_cast<const SLocalApply*>(get_concret(
											*gi))) {
...
GSDD _GShom::has_image_skip(const GSDD &d) const {
	if (d == GSDD::null) {
		return d;
	} else if (d == GSDD::one) {
...
GSDD _GShom::eval_skip(const GSDD &d) const {
	if (d == GSDD::null) {
		return GSDD::null;
	} else if (d == GSDD::one) {
		// basic case, mustn't call d.variable()
	...
	// std::cout << "SEQUENTIAL" << std::endl;
		...
		for (GSDD_DataSet_map::const_iterator it = res.begin(); it != res.end();
				++it) {
...
GShom _GShom::compose(const GShom &r) const {
	//return GHom(this) & r;
...
GSDD StrongShom::has_image(const GSDD &d) const {
...
GShom::GShom(const MLShom &h) :
		concret(canonical(sns::MLShomAdapter(h))) {
}
;

GShom::GShom(const GSDD &d) :
		concret(canonical(sns::Constant(d))) {
}
...
void GShom::garbage() {
	...
	// mark phase
	for (UniqueTable<_GShom>::Table::iterator di = canonical.table.begin();
			di != canonical.table.end(); ++di) {
		(*di)->mark_if_refd();
	}
	// sweep phase
	for (UniqueTable<_GShom>::Table::iterator di = canonical.table.begin();
			di != canonical.table.end();) {
		if (!(*di)->is_marked()) {
...
Shom::Shom(int var, const DataSet &val, const GShom &h) :
		GShom(var, val, h) {
	concret->ref();
}
...
GShom fixpoint(const GShom &h, bool is_top_level) {
	...
	// is it the fixpoint of an union ?
	if (const sns::Add *add = dynamic_cast<const sns::Add*>(_GShom::get_concret(
			h))) {
		// Check if we have (sel & F + id) where sel is a selector and F is a sum
		if (add->parameters.size() == 2) {
			GShom other;
			bool haveId = false;
			for (sns::Add::parameters_it it = add->parameters.begin();
					it != add->parameters.end(); ++it) {
				if (*it == GShom::id)
					haveId = true;
				else
					other = *it;
			...
			if (haveId) {
				...
				if (const sns::Compose *comp =
						dynamic_cast<const sns::Compose*>(_GShom::get_concret(
								other))) {
...
//		trace << "Hit matches second criterion sel & Add ! " << std::endl;
						...
						if (!selr.empty()) {
							...
							for (sns::Add::parameters_it it =
									subadd->parameters.begin();
									it != subadd->parameters.end(); ++it) {
							...
							if (!doC.empty()) {
								...
								if (isLeftSel) {
									...
									finalU.insert(
											sns::Fixpoint(
													(GShom::add(notC) & selector)
															+ GShom::id));
									finalU.insert(
											fixpoint(GShom::add(doC))
													& selector);
								}
	...
	if (is_top_level) {
		if (const sns::LocalApply *lapp =
				dynamic_cast<const sns::LocalApply*>(_GShom::get_concret(h))) {
		...
		} else if (const sns::SLocalApply *lapp =
				dynamic_cast<const sns::SLocalApply*>(_GShom::get_concret(h))) {
...
static void addParameter(const GShom &hh, std::map<int, GHom> &local_homs,
		std::map<int, GShom> &local_shoms, d3::set<GShom>::type &parameters,
		bool &have_id) {
	const _GShom *h = _GShom::get_concret(hh);
	const std::type_info &t = typeid( *h );
	if (t == typeid(sns::Add)) {
		const sns::Add::parameters_t &local_param =
				((const sns::Add*) h)->parameters;
		for (sns::Add::parameters_it it = local_param.begin();
				it != local_param.end(); ++it) {
			addParameter(_GShom::get_concret(*it), local_homs, local_shoms,
					parameters, have_id);
		}
...
static void buildUnionParameters(d3::set<GShom>::type &p,
	...
	for (std::map<int, GShom>::iterator it = local_shoms.begin();
			it != local_shoms.end(); ++it) {
		if (have_id) {
			const std::type_info &t = typeid( *_GShom::get_concret(
							it->second) );
...
static void addCompositionParameter(const GShom &h,
	...
	if (const sns::And *hAnd =
			dynamic_cast<const sns::And*>(_GShom::get_concret(h))) {
		// recursively add each parameter
		for (sns::And::parameters_it it = hAnd->parameters.begin();
				it != hAnd->parameters.end(); ++it) {
			addCompositionParameter(*it, args);
		}
	} else if (const sns::Compose *comp =
			dynamic_cast<const sns::Compose*>(_GShom::get_concret(h))) {
		addCompositionParameter(comp->left, args);
		addCompositionParameter(comp->right, args);
	} else {
		// first test for possible nesting of locals
		if (const sns::LocalApply *lh2 =
				dynamic_cast<const sns::LocalApply*>(_GShom::get_concret(h))) {
			// test for local that can be nested
			for (sns::And::parameters_t::iterator it = args.begin();
					it != args.end(); ++it) {
				if (const sns::LocalApply *lh1 =
						dynamic_cast<const sns::LocalApply*>(_GShom::get_concret(
								*it))) {
		...
		} else if (const sns::SLocalApply *lh2 =
				dynamic_cast<const sns::SLocalApply*>(_GShom::get_concret(h))) {
			// test for local that can be nested
			for (sns::And::parameters_t::iterator it = args.begin();
					it != args.end(); ++it) {
				if (const sns::SLocalApply *lh1 =
						dynamic_cast<const sns::SLocalApply*>(_GShom::get_concret(
								*it))) {
					if (lh1->target == lh2->target) {
						*it = (localApply(lh1->h & lh2->h, lh1->target));
						return;
		...
		for (sns::And::parameters_it it = args.begin(); it != args.end();
				++it) {
			if (commutative(*it, h)) {
				argsC.push_back(*it);
			} else {
		...
		if (argsNOTC.empty()) {
			...
			bool donormal = true;
			if (const sns::Compose *comph1 =
					dynamic_cast<const sns::Compose*>(_GShom::get_concret(h1))) {
...
static void printCondError(const GShom &cond) {
	...
	std::cerr
			<< "Does not have selector flag set to true. If your logic is correct, check that"
...
GShom ITE(const GShom &cond, const GShom &iftrue, const GShom &iffalse) {
	if (!cond.is_selector()) {
		std::cerr << "Creating a complement condition in an ITE construct : "
				<< std::endl;
		printCondError(cond);
		assert(false);
	}
...
GShom operator!(const GShom &cond) {
	if (!cond.is_selector()) {
		std::cerr << "Creating a complement condition with operator! :  ! cond"
				<< std::endl;
		printCondError(cond);
		assert(false);
		return Shom::null;
		...
		return GShom::id;
	} else if (const sns::SLocalApply *lh1 =
			dynamic_cast<const sns::SLocalApply*>(_GShom::get_concret(cond))) {
		return localApply(!lh1->h, lh1->target);
	} else if (const sns::LocalApply *lh1 =
			dynamic_cast<const sns::LocalApply*>(_GShom::get_concret(cond))) {
		return localApply(!lh1->h, lh1->target);
	} else if (const sns::SNotCond *lh1 =
			dynamic_cast<const sns::SNotCond*>(_GShom::get_concret(cond))) {
		return lh1->cond_;
	} else {
...
GShom operator*(const GShom &h, const GShom &cond) {
	if (!cond.is_selector()) {
		std::cerr
				<< "Creating a * intersection between homomorphisms, but second argument is not a selector. "
				<< std::endl;
		printCondError(cond);
		assert(false);
	}
...
void GShom::pstats(bool) {
	std::cout << "*\nGSHom Stats : size unicity table = " << canonical.size()
			<< std::endl;
